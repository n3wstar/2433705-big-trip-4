import { formatStringToDateTime, formatStringToShortDate, formatStringToTime, getDuration } from '../utils.js';

function createOffersTemplate({ offers, pointOffers }) {
  const offerItem = offers.map((offer) => {
    const foundOffer = pointOffers.offers.find((o) => o.id === offer);
    if (foundOffer) {
      return `
...
function createPointMarkup({point, pointDestination, pointOffers}) {
  const { type, offers, basePrice, dateFrom, dateTo, isFavorite } = point;
